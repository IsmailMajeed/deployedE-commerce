...
export default function Nav({ show, children }) {
  const styleLink = 'flex gap-1 p-1 no-underline py-3';
  const inactiveLink = styleLink + ' text-white';
  const activeLink = styleLink + ' bg-highlight text-black rounded-l-md bg-gray-50';
  const inactiveIcon = 'w-6 h-6';
  const activeIcon = inactiveIcon + '';
  const { pathname } = useLocation();

  return (
    <aside className={(show ? 'left-0 ' : '-left-full') + " top-0 z-10 text-gray-500 max-md:h-screen py-4 pl-5 fixed w-1/2 bg-gray-800 md:static md:w-64 min-h-screen transition-all"}>
      <div className={"mb-4 mr-4 relative "}>
        <Logo show={show} />
        {children}
      </div>
      <nav className={"flex flex-col gap-2 " + (show ? '' : '')}>
        ...
        <Link to={'/Featured'} className={pathname === '/Featured' ? activeLink : inactiveLink}>
        ...
        <Link to={'/Settings'} className={pathname.includes('/Settings') ? activeLink : inactiveLink}>
